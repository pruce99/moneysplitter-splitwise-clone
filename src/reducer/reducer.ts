import { Person, Product } from "../Logic/Logic";
...
const initialAppState: IAppState = {
  people: [
    ...
    new Person(2, "Carla", [new Product(102, "Cheese", 10.55)]),
    new Person(3, "Dan"),
    new Person(4, "Eddy", [new Product(103, "Plates", 19.99)]),
    new Person(5, "Fred", [new Product(104, "Knife", 2.49)]),
  ],
...
const appReducer: Reducer<IAppState> = (
  ...
  switch (action.type) {
      ...
      newState.people.forEach((person) => {
        ...
        if (idx > -1) {
          ...
          if (action.setSelectedOwner) {
            let newPerson: null | Person =
              Person.findById(state.selectedPersonId || -1, newState.people) ||
              null;
            if (newPerson) {
    ...
    case DELETE_PERSON:
      ...
      const idx = newState.people.findIndex(
    ...
    case ADD_PRODUCT:
      ...
      let person: Person | undefined = newState.people.find(
        (person) => person.id === state.selectedPersonId
      );
      if (!person) return state;
      person.products.push(
        new Product(
          person.id * 1000 + person.products.length,
          action.name,
          action.value
        )
      );
