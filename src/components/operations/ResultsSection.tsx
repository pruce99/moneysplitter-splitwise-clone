  ...
  public render() {
    ...
    return (
      <div>
        ...
        <h3>
          <small>
            Cost per person: {result.costPerPerson.toFixed(2)} $/person
          </small>
        </h3>
        ...
        <div style={{ marginTop: 25 }}>{this.renderTotals(result)}</div>
      </div>
    );
  }
  ...
  private renderTotals(result: ICalculationResult): JSX.Element[] {
    return result.totals.map((t, idx) => (
      <p
        key={idx}
        className={
          t.type === TransactionType.GIVE ? "text-danger" : "text-success"
        }
      >
        {`${t.person.name} has to ${
          t.type === TransactionType.GIVE ? "give" : "receive"
        } ${t.amount.toFixed(2)} $ in total`}
      </p>
    ));
  }
